...
//				   : $lucro * $porcentagem_freteiro  ;
			$frete = ($vale['ItemPedido']['frete']) ?:($vale['Vale']['motorista_tipo'] == 1 )? 
				   ( $vale['Vale']['periodo_id'] == 1 ? $preco_diurno : $preco_noturno)
				   : $lucro * $porcentagem_freteiro  ;
...
//			pr($lucro);
			...
			$tcpdf->Cell($col[$i++], 0, $vale['ItemPedido']['pedido_id'],0,0,'',true);
			$tcpdf->Cell($col[$i++], 0, substr($data,0,2).'/'.$mes[(int)substr($data,3,2)-1],0,0,'',true);
			$tcpdf->Cell($col[$i++], 0, limitarString($vale['Fornecedor']['nome'],10),0,0,'',true);
			$tcpdf->Cell($col[$i++], 0, limitarString($vale['ItemPedido']['Produto']['nome'], 10),0,0,'',true);
			$tcpdf->Cell($col[$i++], 0, moedaBr($vale['ItemPedido']['valor_total']),0,0,'',true);
			$tcpdf->Cell($col[$i++], 0, limitarString($vale['ItemPedido']['Pedido']['Cliente']['nome'], 15).' - '.limitarString($vale['ItemPedido']['Pedido']['Obra']['endereco'], 20),0,0,'',true);
			$tcpdf->Cell($col[$i++], 0, moedaBr($vale['ItemPedido']['valor_total']/$vale['ItemPedido']['quantidade']),0,0,'R',true);
